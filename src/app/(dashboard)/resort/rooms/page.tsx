...
export default function ResortRoomsPage() {
  ...
  const handleToggleActive = async (roomId: string, currentStatus: boolean) => {
    try {
      ...
      toast.success(
        !currentStatus ? "Villa is now visible to tourists" : "Villa marked hidden"
      );
    } catch {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-neutral-200 pb-5">
        <div>
          ...
          <p className="text-xs sm:text-sm text-neutral-600 mt-1">
            Configure luxury villas, beachfront suites, per-night rates, and guest capacity
          </p>
        </div>
      ...
      {!property ? (
        <EmptyState
          icon={Building2}
          title="Create your resort profile first"
          description="You need to set up your resort profile before adding room inventory."
          actionLabel="Create Resort Profile"
          onAction={() => setIsPropertyModalOpen(true)}
        />
      ) : rooms.length === 0 ? (
        <EmptyState
          icon={BedDouble}
          title="No rooms or villas configured"
          description="Start receiving bookings by adding your first villa with amenities and rates in Philippine Peso."
          actionLabel="Add Villa / Room"
          onAction={() => {
            setEditingRoom(null);
            setIsRoomModalOpen(true);
          }}
